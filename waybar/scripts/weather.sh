...
# output for Waybar
if [ -n "$text" ] && ! echo "$text" | grep -q "Unknown location"; then
    # Escape special characters for JSON
    text_escaped=$(echo "$text" | sed 's/\\/\\\\/g' | sed 's/"/\\"/g')
    echo "{\"text\": \"$text_escaped\", \"tooltip\": \"<tt>$tooltip</tt>\", \"class\": \"weather\"}"
else
    # Fallback output when location is unknown
    echo "{\"text\": \" N/A\", \"tooltip\": \"Location unknown\", \"class\": \"weather-error\"}"
fi
